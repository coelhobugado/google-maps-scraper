...
import (
	...
	"github.com/coelhobugado/google-maps-scraper/leadsdb"
	...
	if r.cfg.GridBBox != "" {
		...
		seedJobs, err = runner.CreateGridSeedJobs(
			...
			&runner.RequestBudget{
				MaxGridCells: r.cfg.MaxGridCells,
				MaxSeedJobs:  r.cfg.MaxSeedJobs,
			},
		)
	} else {
...
func (r *fileRunner) setWriters() error {
	...
	case r.cfg.CustomWriter != "":
		customWriter, err := runner.LoadCustomWriter(r.cfg.CustomWriter, "")
		if err != nil {
			return err
		}

		r.writers = append(r.writers, customWriter)
	case r.cfg.LeadsDBAPIKey != "" || r.cfg.LeadsDBPreviewMode:
		r.writers = append(r.writers, leadsdb.NewWithConfig(leadsdb.Config{
			APIKey:      r.cfg.LeadsDBAPIKey,
			PreviewMode: r.cfg.LeadsDBPreviewMode,
		}))
	default:
	...
	for i, w := range r.writers {
		r.writers[i] = runner.NewDriftDetectorWriter(w)
	}
