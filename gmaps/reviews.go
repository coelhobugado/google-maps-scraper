...
func (f *fetcher) fetch(ctx context.Context) (FetchReviewsResponse, error) {
	...
	if f.params.page != nil {
		...
		log.Warn("Browser-based RPC fetch failed, trying HTTP", "error", err)
	}

	// Fallback to direct HTTP (may fail due to lack of authentication)
	currentPageBody, err := f.fetchReviewPage(ctx, reviewURL)
	if err != nil {
		log.Warn("RPC fetch failed, will try DOM extraction", "error", err)
	...
	for nextPageToken != "" && len(ans.pages) < maxReviewPages {
		reviewURL, err = f.generateURL(f.params.mapURL, nextPageToken, reviewPageSize, requestIDForSession)
		if err != nil {
			log.Warn("review pagination URL generation failed", "error_code", "review_url")
			break
		}

		currentPageBody, err = f.fetchReviewPage(ctx, reviewURL)
		if err != nil {
			log.Warn("review page fetch failed", "error_code", "review_fetch")
			break
		}
...
func (f *fetcher) fetchWithBrowser(ctx context.Context, initialURL, requestID string) (FetchReviewsResponse, error) {
	...
	encodedInitialURL, _ := json.Marshal(initialURL)
	// Use JavaScript fetch to get the reviews with proper cookies. URL is JSON-escaped.
	...
	for nextPageToken != "" && len(ans.pages) < maxReviewPages {
		...
		encodedNextURL, _ := json.Marshal(nextURL)
		jsCode = fmt.Sprintf(`async () => {
			try {
				const response = await fetch(%s, {
					method: 'GET',
...
func extractPlaceID(mapURL string) (string, error) {
	...
	return "", errors.New("could not extract place ID from map URL")
}
...
func (f *fetcher) fetchReviewPage(ctx context.Context, u string) ([]byte, error) {
	...
	if resp.Error != nil {
		return nil, fmt.Errorf("review fetch failed: %w", resp.Error)
	}

	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("review fetch returned status %d", resp.StatusCode)
	}
...
func extractReviewsFromPage(ctx context.Context, page scrapemate.BrowserPage) ([]DOMReview, error) {
	log.Info("Attempting DOM-based review extraction")
	...
	if clickedReviews != nil && clickedReviews != false {
		log.Debug("Clicked reviews", "via", clickedReviews)
	}
	...
	for attempt := 0; attempt < maxScrollAttempts; attempt++ {
		...
		if err != nil {
			log.Error("Error extracting reviews from DOM", "error", err)
		...
		if currentCount == lastCount {
			stuckCount++
			if stuckCount > 5 {
				log.Warn("Review count stuck, stopping scroll", "count", currentCount)
				break
			}
		} else {
			stuckCount = 0
	...
	log.Info("DOM extraction completed", "reviews_found", len(reviews))
...
func FetchReviewsWithFallback(ctx context.Context, params fetchReviewsParams) (FetchReviewsResponse, []DOMReview, error) {
	...
	if err == nil && len(rpcResponse.pages) > 0 {
		...
		if totalReviews > 0 {
			log.Info("RPC extraction successful", "pages", len(rpcResponse.pages), "reviews", totalReviews)
			return rpcResponse, nil, nil
		}

		log.Info("RPC returned empty reviews, trying DOM extraction")
	}
	...
	if params.page != nil {
		...
		if domErr == nil && len(domReviews) > 0 {
			log.Info("DOM extraction successful", "reviews", len(domReviews))
			return FetchReviewsResponse{}, domReviews, nil
		}

		if domErr != nil {
			log.Error("DOM extraction failed", "error", domErr)
		}
	}
