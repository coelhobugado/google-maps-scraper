...
func (e *SchemaDriftError) Error() string {
	if e == nil || e.Stage == "" {
		return "Google Maps response schema changed"
	}
	return "Google Maps response schema changed during " + e.Stage
}
...
func (e *Entry) IsWebsiteValidForEmail() bool {
	if e.WebSite == "" || isSocialProfileURL(e.WebSite) {
		return false
	}
	return true
}
...
func isSocialProfileURL(raw string) bool {
	raw = strings.TrimSpace(raw)
	if raw != "" && !strings.Contains(raw, "://") {
		raw = "https://" + raw
	}
...
func websiteContacts(raw string) (website, instagram string) {
	raw = strings.TrimSpace(raw)
	if isInstagramURL(raw) {
		return "", raw
	}
	if isSocialProfileURL(raw) {
		return "", ""
	}
	return raw, ""
}
...
func (e *Entry) CsvRow() []string {
	website, instagram := websiteContacts(e.WebSite)
	return []string{
		e.ID,
		e.Link,
		e.Title,
...
func extractReviews(data []byte) []Review {
	...
	if err := json.Unmarshal(data, &jd); err != nil {
		log.Debug("Error unmarshalling RPC JSON", "error", err, "data_len", len(data))
		return nil
	}

	if len(jd) < 3 {
		log.Debug("RPC response has too few elements", "elements", len(jd), "expected", "3+")
		return nil
	}
...
func findDataArray(jd []any) ([]any, error) {
	// First check the typical index
	if len(jd) > 6 {
		if arr, ok := jd[6].([]any); ok && len(arr) >= 20 {
			if title, _ := arr[11].(string); title != "" {
				return arr, nil
			}
		}
	}

	// Scan if the index shifted (schema drift)
	for _, el := range jd {
		arr, ok := el.([]any)
		if !ok || len(arr) < 20 {
			continue
		...
		if title, _ := arr[11].(string); title != "" {
			if coords, ok := arr[9].([]any); ok && len(coords) >= 4 {
				if _, okLat := coords[2].(float64); okLat {
					if _, okLon := coords[3].(float64); okLon {
						return arr, nil
					}
				}
			}
	...
	return nil, &SchemaDriftError{Stage: "place-data discovery"}
}
...
func EntryFromJSON(raw []byte, reviewCountOnly ...bool) (entry Entry, err error) {
	defer func() {
		if recover() != nil {
			err = &SchemaDriftError{Stage: "place-data decoding"}
		}
	}()
	...
	jd, decodeErr := decodeInitializationState(raw)
	if decodeErr != nil {
		return entry, decodeErr
	}

	darray, err := findDataArray(jd)
	if err != nil {
		return entry, err
	}
	...
	if validationErr := validateDecodedEntry(&entry, onlyReviewCount); validationErr != nil {
		return entry, validationErr
	}
	return entry, nil
}

func decodeInitializationState(raw []byte) ([]any, error) {
	if len(raw) == 0 {
		return nil, &SchemaDriftError{Stage: "empty response"}
	}
	var data []any
	if err := json.Unmarshal(raw, &data); err != nil {
		return nil, fmt.Errorf("decode Google Maps response: %w", err)
	}
	if len(data) == 0 {
		return nil, &SchemaDriftError{Stage: "root validation"}
	}
	return data, nil
}

func validateDecodedEntry(entry *Entry, reviewCountOnly bool) error {
	if reviewCountOnly {
		return nil
	}
	if entry.Title == "" || math.IsNaN(entry.Latitude) || math.IsNaN(entry.Longitude) {
		return &SchemaDriftError{Stage: "required-field validation"}
	}
	if entry.Latitude < -90 || entry.Latitude > 90 || entry.Longitude < -180 || entry.Longitude > 180 {
		return &SchemaDriftError{Stage: "coordinate validation"}
	}
	return nil
}
