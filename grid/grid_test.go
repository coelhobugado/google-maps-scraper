...
func TestGridCrossesAntimeridianAndRespectsLimit(t *testing.T) {
	bbox, err := grid.ParseBoundingBox("-1,179,1,-179")
	if err != nil {
		t.Fatal(err)
	}
	count := grid.EstimateCellCount(bbox, 100)
	if count <= 0 || count > 20 {
		t.Fatalf("unexpected count %d", count)
	}
	cells, err := grid.GenerateCellsLimited(bbox, 100, count)
	if err != nil {
		t.Fatal(err)
	}
	if len(cells) != count {
		t.Fatalf("len=%d count=%d", len(cells), count)
	}
	if _, err := grid.GenerateCellsLimited(bbox, 1, 2); err == nil {
		t.Fatal("expected cell limit error")
	}
}
